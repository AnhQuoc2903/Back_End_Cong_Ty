...
async function seed() {
    ...
    for (const name of permNames) {
        let p = await permission_model_1.Permission.findOne({ name });
        if (!p) {
            p = await permission_model_1.Permission.create({ name, description: name });
            console.log(`Created permission: ${name}`);
        }
        permissions.push(p);
    }
    // 2) Role Admin: có tất cả quyền
    let adminRole = await role_model_1.Role.findOne({ name: "Admin" });
    const allPermIds = permissions.map((p) => p._id.toString());
    if (!adminRole) {
        adminRole = await role_model_1.Role.create({
            name: "Admin",
        ...
        const currentIds = (adminRole.permissions || []).map((id) => id.toString());
    ...
    let staffRole = await role_model_1.Role.findOne({ name: "Staff" });
    ...
    if (!staffRole) {
        staffRole = await role_model_1.Role.create({
            name: "Staff",
        ...
        const currentIds = (staffRole.permissions || []).map((id) => id.toString());
        const merged = Array.from(new Set([...currentIds, ...staffPermIds]));
        staffRole.permissions = merged;
        await staffRole.save();
        console.log("Updated role: Staff (merged permissions)");
    }
    // 4) Admin user
    let adminUser = await user_model_1.User.findOne({ email: "admin@example.com" });
    if (!adminUser) {
        const passwordHash = await (0, password_1.hashPassword)("123456");
        ...
        const roleIds = (adminUser.roles || []).map((r) => r.toString());
        if (!roleIds.includes(adminRole._id.toString())) {
            const mergedRoles = Array.from(new Set([...roleIds, adminRole._id.toString()]));
            adminUser.roles = mergedRoles;
    ...
    for (const c of sampleCategories) {
        const existed = await artifactCategory_model_1.ArtifactCategory.findOne({ name: c.name });
        if (!existed) {
            await artifactCategory_model_1.ArtifactCategory.create(c);
